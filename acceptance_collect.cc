...
// Outputs
#define dirroot "/sps/compass/julien/nicolas/Multiplicities/acceptance"
...
int main(int argc, char **argv)
{
  ...
  while(periods >> periodName)
  {
    ...
    ofstream ofs_h(Form("%s/%d/acceptance_hadron_%s.txt",dirroot,year,periodName.c_str()), std::ofstream::out | std::ofstream::trunc);
    ofstream ofs_p(Form("%s/%d/acceptance_pion_%s.txt",dirroot,year,periodName.c_str()), std::ofstream::out | std::ofstream::trunc);
    ofstream ofs_k(Form("%s/%d/acceptance_kaon_%s.txt",dirroot,year,periodName.c_str()), std::ofstream::out | std::ofstream::trunc);
    ofstream ofs_pr(Form("%s/%d/acceptance_proton_%s.txt",dirroot,year,periodName.c_str()), std::ofstream::out | std::ofstream::trunc);
        ...
        for(int m=0; m<2; m++)
        {
          ...
          for(int j=0; j<6; j++)
          {
              ...
              for(int k=0; k<12; k++)
              {
                ...
                // cout << "fNDIS_evt[0][m][i][j][k] = " << fNDIS_evt[0][m][i][j][k] << endl;
                // cout << "fNDIS_evt_MC[0][m][i][j][k] = " << fNDIS_evt_MC[0][m][i][j][k] << endl;
                // cout << "fGnrt[i][j][k].tab[c][m][0][0] = " << fGnrt[i][j][k].tab[c][m][0][0] << endl;
                // cout << "fAcceptance[i][j][k].tab[c][m][0][0] = " << fAcceptance[i][j][k].tab[c][m][0][0] << endl;
    ...
    for(int i=0; i<9; i++)
    {
      for(int j=0; j<6; j++)
      {
        for(int k=0; k<12; k++)
        {
            for(int m = 0; m<2; m++)
            {
              ofs_h << fAcceptance[i][j][k].tab[1][m][0][3] << " " << fAcceptance[i][j][k].tab[1][m][1][3] << " ";
              ofs_h << fAcceptance[i][j][k].tab[0][m][0][3] << " " << fAcceptance[i][j][k].tab[0][m][1][3] << endl;

              ofs_p << fAcceptance[i][j][k].tab[1][m][0][0] << " " << fAcceptance[i][j][k].tab[1][m][1][0] << " ";
              ofs_p << fAcceptance[i][j][k].tab[0][m][0][0] << " " << fAcceptance[i][j][k].tab[0][m][1][0] << endl;

              ofs_k << fAcceptance[i][j][k].tab[1][m][0][1] << " " << fAcceptance[i][j][k].tab[1][m][1][1] << " ";
              ofs_k << fAcceptance[i][j][k].tab[0][m][0][1] << " " << fAcceptance[i][j][k].tab[0][m][1][1] << endl;

              ofs_pr << fAcceptance[i][j][k].tab[1][m][0][2] << " " << fAcceptance[i][j][k].tab[1][m][1][2] << " ";
              ofs_pr << fAcceptance[i][j][k].tab[0][m][0][2] << " " << fAcceptance[i][j][k].tab[0][m][1][2] << endl;
            }
        }
      }
    }
